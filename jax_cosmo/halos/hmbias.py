...
class HaloBiasBhattacharya11:
    ...
    def compute_bias(self, cosmology: Cosmology, sigma_mass: float, scale_factor: float) -> float:
        """
        ...
        bias_numerator = scaled_peak_height_sq - self.bias_offset + (2 * self.power_law_index / power_law_term)
...
class HaloBiasSheth01:
    ...
    def compute_bias(self, cosmology: Cosmology, sigma_mass: float, scale_factor: float) -> float:
        ...
        correction_term = self.bias_coefficient * (1.0 - self.exponent) * (1.0 - 0.5 * self.exponent)

        first_term = self.sqrt_scaling_factor * scaled_peak_height_sq * (1 + self.bias_coefficient / exponent_term)
...
class HaloBiasSheth99:
    ...
    def compute_bias(self, cosmology: Cosmology, sigma_mass: float, scale_factor: float) -> float:
        ...
        correction_term = 1 + 2 * self.power_law_index / (1 + scaled_peak_height_sq ** self.power_law_index)
...
class HaloBiasTinker10:
    ...
    def compute_bias(self, cosmology: Cosmology, sigma_mass: float, scale_factor: float) -> float:
        ...
        """
        peak_height = self.critical_density / sigma_mass
        log_overdensity = jnp.log10(self.mass_definition.convert_to_matter_overdensity(cosmology, scale_factor))
        ...
        bias = 1 - coefficient_a * peak_height_scaled / (peak_height_scaled + self.critical_density ** exponent_a)
        bias += self.bias_coefficient * peak_height ** self.exponent_b + coefficient_c * peak_height ** self.exponent_c
