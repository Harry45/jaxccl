...
def fx_projected_nfw(x: jnp.ndarray) -> jnp.ndarray:
    ...
    xf = x.flatten()
    return jnp.piecewise(xf, [xf < 1, xf > 1], [f1, f2, 1.0 / 3.0]).reshape(x.shape)
...
def fx_cumul2d_nfw(x: jnp.ndarray) -> jnp.ndarray:
    ...
    f = jnp.piecewise(xf, [xf < 1, xf > 1], [f1, f2, omln2]).reshape(x.shape)
    return 2 * f / x**2
...
class JAXHaloProfileNFW:
    ...
    def __init__(
        ...
        if projected_analytic:
            if truncated:
                raise ValueError(
                    "Analytic projected profile not supported for truncated NFW. "
        ...
        if cumul2d_analytic:
            if truncated:
                raise ValueError(
                    "Analytic cumulative 2D profile not supported for truncated NFW. "
