...
__all__ = ["smail_nz", "kde_nz", "delta_nz", "custom_nz"]
...
class custom_nz(redshift_distribution):
    """
    The redshift distributions are generally provided, as part of the release of cosmological data.
    The goal is to essentially interpolate the redshift distribution at any point within the domain.
    """

    def pz_fn(self, redshifts: np.ndarray) -> np.ndarray:
        """
        Generates a distribution of the redshift distribution simply by interpolation.

        Args:
            redshifts (np.ndarray): the set of redshifts for which we want the p(z)

        Returns:
            np.ndarray: the probability density function at the queried redshift
        """
        zcat, probz = self.params[0:2]
        nz_queried = np.interp(redshifts, zcat, probz)
        return nz_queried
