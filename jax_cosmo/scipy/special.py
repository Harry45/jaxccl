...
def compute_sici(xvalues: jnp.ndarray,
                 nterms: int = 12,
                 npoints: int = 2000,
                 xmin: float = 1e-5) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
    ...
    def large_x_case(x):
        ...
        even = jnp.sum(ratio[0::2] * jnp.tile(jnp.array([1.0, -1.0]), len(ratio[0::2]) // 2))
        odd = jnp.sum(ratio[1::2] * jnp.tile(jnp.array([1.0, -1.0]), len(ratio[1::2]) // 2))
    ...
    def compute_single(x):
        return lax.cond(x < 10.0, lambda x: small_x_case(x), lambda x: large_x_case(x), x)
